...
def cross_validate_model(rank, world_size, args, dataset, n_splits=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
       ...
       # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        ...
        train_loader = torch.utils.data.DataLoader(train_subset, batch_size=args.batch,pin_memory=True, shuffle= False, sampler = DistributedSampler(dataset),drop_last=True)
        val_loader = torch.utils.data.DataLoader(val_subset, batch_size=args.batch,pin_memory=True, shuffle= False, sampler = DistributedSampler(dataset),drop_last=True)
