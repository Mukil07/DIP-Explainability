...
def train(args, train_dataloader, valid_dataloader, model, criterion1, criterion2, criterion3, optimizer, device,writer,fold):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (img1,img2,cls,gaze,ego) in tqdm(enumerate(train_dataloader)):
            ...
            optimizer.zero_grad()
            print(f"Rank {dist.get_rank()} reached loss.backward()")
            loss.backward()
            print(f"Rank {dist.get_rank()} completed loss.backward()")
        ...
        # At the end of the epoch, gather data across all processes
        # all_preds_local = torch.cat(all_preds)
        # all_labels_local = torch.cat(all_labels)

        all_preds_local = torch.cat(all_preds).to(device)
        all_labels_local = torch.cat(all_labels).to(device)

        print(all_preds_local.shape)
        print(all_labels_local.shape)
        ...
        print("empty holder", len(all_preds_gathered))
        dist.barrier()
