...
def decode_video(video_path, start,end, rand_load, num_frames=16):
    flag = False
    ...
    for num_ in range(int(start_time), int(end_time), int(interval*rand_load)):

        for j in range(rand_load): 
            x =  random.randint(0,int(interval*rand_load)-1)
            i = num_+x
...
class CustomDataset(Dataset):
    def __init__(self,csv_type, rand_load = 1, debug=None, transform=None):
    ...
    def __getitem__(self, idx):
        while True:
            ...
            fl1,fl2= False,False
            video_frames1,fl1 = decode_video(face_path, start, end, self.rand_load)
            video_frames2,fl2 = decode_video(road_path, start, end, self.rand_load )
