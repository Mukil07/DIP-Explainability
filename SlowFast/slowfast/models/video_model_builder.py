...
class MViT_Adapter(nn.Module):
    ...
    def __init__(self,cfg):
        super(MViT_Adapter, self).__init__()  
        embed_dim = cfg.CBM.EMBED_DIM
        num_classes = cfg.CBM.NUM_CLASSES
        multitask_classes = cfg.CBM.MUL_CLASSES
        multitask = cfg.CBM.MULTITASK
        n_attributes = cfg.CBM.N_ATTR
        bottleneck = cfg.CBM.BOTTLENECK
        expand_dim = cfg.CBM.EXP_DIM
        ...
        self.final_model = cbm_module(self.first_model, embed_dim, num_classes, multitask_classes, multitask, n_attributes, bottleneck, expand_dim,
                 use_relu, use_sigmoid,connect_CY)
        
    def forward(self,img1,img2):
        #import pdb;pdb.set_trace()
        return self.final_model(img1,img2)