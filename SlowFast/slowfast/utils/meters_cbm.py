...
class ValMeter:
    ...
    def __init__(self, max_iter, cfg):
        ...
        self.accuracy_gaze = 0.0
        self.f1score_gaze = 0.0
    ...
    def update_predictions(self, preds, labels, gaze, ego):
        """
        ...
        if self._cfg.CBM.GAZE_CBM:
            predicted_gaze = torch.argmax(preds[2],dim=1)
            self.all_preds_gaze.append(predicted_gaze.cpu())
            self.all_labels_gaze.append(gaze.cpu())   
            predicted_ego = (torch.sigmoid(preds[1]) > 0.5).float().cpu()
            self.all_preds_ego.append(predicted_ego)
            self.all_labels_ego.append(torch.vstack(ego).to(dtype=torch.float).permute((-1,-2)).cpu())


        elif self._cfg.CBM.EGO_CBM:

            predicted_gaze = torch.argmax(preds[1],dim=1)
            self.all_preds_gaze.append(predicted_gaze.cpu())
            self.all_labels_gaze.append(gaze.cpu())    
            ...
            self.all_preds_gaze.append(predicted_gaze.cpu())
            self.all_labels_gaze.append(gaze.cpu())          
            predicted_ego = (torch.sigmoid(torch.hstack(preds[16:33])) > 0.5).float().cpu()
            self.all_preds_ego.append(predicted_ego)
            self.all_labels_ego.append(torch.vstack(ego).to(dtype=torch.float).permute((-1,-2)).cpu()) 
        
        elif self._cfg.CBM.MULTITASK:

            predicted_gaze = torch.argmax(preds[1],dim=1)
            self.all_preds_gaze.append(predicted_gaze.cpu())
            self.all_labels_gaze.append(gaze.cpu()) 
            
            predicted_ego = (torch.sigmoid(preds[2]) > 0.5).float().cpu()
            self.all_preds_ego.append(predicted_ego)
            self.all_labels_ego.append(torch.vstack(ego).to(dtype=torch.float).permute((-1,-2)).cpu())        
        ...
        if self._cfg.DATA.MULTI_LABEL:
            ...
            if self._cfg.CBM.EGO_CBM or self._cfg.CBM.COMB_BOTTLE or self._cfg.CBM.MULTITASK or self._cfg.CBM.GAZE_CBM:
                ...
                if self.min_top1_err < top1_err:
                    ...
                    self.accuracy_gaze = accuracy_val_gaze
                ...
                # self.f1score_gaze = max(self.f1score_ego,f1_val_ego)
