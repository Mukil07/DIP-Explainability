...
class EarlyFusion(VideoMAEPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings1 = VideoMAEEmbeddings(config)
        self.embeddings2 = VideoMAEEmbeddings(config)
    ...
    def get_input_embeddings(self):
        ...
        return self.embeddings1.patch_embeddings
    ...
    def forward(
        ...
        head_mask = self.get_head_mask(head_mask, self.config.num_hidden_layers)
        #import pdb;pdb.set_trace()
        embedding_output1 = self.embeddings1(pixel_values1, bool_masked_pos)
        embedding_output2 = self.embeddings2(pixel_values2, bool_masked_pos)

        embedding_output = torch.cat([embedding_output1,embedding_output2],dim=1)
        #import pdb;pdb.set_trace()
        encoder_outputs = self.encoder(
            embedding_output,
            head_mask=head_mask,
...
class VideoMAEForVideoClassification2(VideoMAEPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.num_labels = config.num_labels
        self.videomae = EarlyFusion(config)
    ...
    def forward(
        self,
        pixel_values1: Optional[torch.Tensor] = None,
        pixel_values2: Optional[torch.Tensor] = None,
        head_mask: Optional[torch.Tensor] = None,
        labels: Optional[torch.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
