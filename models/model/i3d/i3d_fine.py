...
class CBM(InceptionI3d):
    ...
    def __init__(self, num_classes=5, n_attributes=17, bottleneck=True, expand_dim=512, connect_CY=False, dropout_keep_prob=0.45, clusters=5, batch=8):
...
def ModelXtoCtoY_fine(num_classes, multitask_classes, multitask, n_attributes, bottleneck, expand_dim,
                 use_relu, use_sigmoid,connect_CY, dropout, clusters, batch):

    model1 = CBM(num_classes=num_classes,n_attributes=n_attributes,
                  bottleneck=bottleneck, expand_dim=expand_dim,connect_CY=connect_CY, dropout_keep_prob = dropout, clusters=clusters, batch=batch)
